    ...
    try {
        ...
        const proveedor = await proveedorModel.aggregate([{
            $lookup: {
                ...
                localField: 'ajsnAlmacen.idProducto', //En que campo lo tengo
                foreignField: '_id',
                as: 'ProAlmacen'
            }
        },
                ...
                'InfoAlmacen': {
                    $map: {
                        ...
                        in: {
                            ...
                            'Categoria': '$$almacen.strCategoria',
                            'producto': {
                                $arrayElemAt: [{
                                    $filter: {
                                        input: '$ProAlmacen',
...
app.patch('/', async(req, res) => {
    ...
    try{
        ...
        const transaccionResultado = await session.withTransaction(async ()=>{
            ...
            //await TiendaModel.updateMany([{},{$push:{'arrProveedores':proveedor._id}}],{session:session})
        });
        if(transaccionResultado){
            //const proveedorid= await proveedorModel.findOne({}).sort({_id:-1})
            //const tiendaModel = await TiendaModel.updateMany({},{$push:{'arrProveedores':proveedorid._id}});
            //await TiendaModel.updateMany({},{$push:{'arrProveedores':idProveedor}});
            await TiendaModel.updateMany({},{$push:{'arrProveedores':proveedor}});
            res.status(200).send({
                estatus: '200',
                err: false,
                msg: 'Informacion insertada correctamente.',
